<?php
    ...
    use Dompdf\Dompdf;
    $dompdf = new Dompdf();
    $query = mysqli_query($koneksi,"SELECT * FROM siswa INNER JOIN tb_pinjam_umum_siswa ON tb_pinjam_umum_siswa.id_siswa=siswa.id_siswa WHERE status='Disetujui' ORDER BY tb_pinjam_umum_siswa.id_pumum DESC");
    ...
    $html = '<div style="background-color: #0984b2; color:white; padding:15px;"><center><h3>Data Peminjaman Buku Umum Siswa Pada Perpustakaan <br> MAN 1 PADANG PARIAMAN</h3></center> <br>Periode : Semua Data Peminjaman</div>'.'<hr/>';    
