<?php
    ...
    use Dompdf\Dompdf;
    $dompdf = new Dompdf();
    $query = mysqli_query($koneksi,"SELECT * FROM tb_pinjam_umum_pegawai INNER JOIN pegawai ON pegawai.id_pegawai=tb_pinjam_umum_pegawai.id_pegawai WHERE status='Disetujui' AND tb_pinjam_umum_pegawai.tanggal BETWEEN '$tglAwal' AND '$tglAkhir' ORDER BY tb_pinjam_umum_pegawai.id_pumum DESC");
    ...
    $html = '<div style="background-color: #0984b2; color:white; padding:15px;"><center><h3>Data Peminjaman Buku Umum Pegawai Pada Perpustakaan <br> MAN 1 PADANG PARIAMAN</h3></center><br>Periode : '. $tglAwal . ' s/d ' . $tglAkhir.'</div>'.'<hr/>';        
