<?php
    ...
    use Dompdf\Dompdf;
    $dompdf = new Dompdf();
    $query = mysqli_query($koneksi,"SELECT * FROM siswa INNER JOIN tb_pinjam_koleksi_siswa ON tb_pinjam_koleksi_siswa.id_siswa=siswa.id_siswa WHERE status='Disetujui'");
    ...
    $html = '<div style="background-color: #0984b2; color:white; padding:15px;"><center><h3>Data Peminjaman Buku Koleksi Siswa Pada Perpustakaan <br> MAN 1 PADANG PARIAMAN</h3></center> <br>Periode: Semua Data Peminjaman</div>'.'<hr/>';    
    ...
    while($row = mysqli_fetch_array($query))
    {
        if($row['pengembalian'] == 'Disetujui') {
            $pengembalian = 'Sudah dikembalian';
        }else {
